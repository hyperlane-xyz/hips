...
- There are no refunds for overpayments. Because all data and calculation happens off chain, when a payment is made on the origin chain there is no way to determine if a message has been overpaid for. After a message has been relayed, the relayer could potentially refund overpayments, but this incurs a transaction fee. It's also not very clear which address should receive the refund - this would need to be specified explicitly when making the gas payment.
...
- A transaction won't revert if there is insufficient payment for a message that is being dispatched. With on chain fee quoting, the transaction can be reverted if there is insufficient payment, creating a very strong feedback loop to users. Sending a message should “just work.”
- Generally, it's a rough user experience. Application developers have an easier time reasoning about and dealing with on-chain data, and want to have certainty that messages will be processed. Requiring off-chain fee calculation limits the composability of Hyperlane.

### **Tech Spec**

Paying for fees can be done by paying an `InterchainGasPaymaster` contract relating to an off-chain relayer that will enter a social contract to process a message. This contract is specific to a relayer operator who has the ability to claim fees paid to the contract.
...
interface FeeQuotingIgp {
     ...
     * @dev The `_ismAndHandleGas` should cover ISM metadata calldata gas, ISM.verify gas,
     ...
     * @dev Emits the `GasPayment` event with the total amount of gas paid for.
     * @param _messageId The message ID.
     * @param _destinationDomain The destination domain of the message.
...
contract IsmFeeQuotingIgp {
    ...
    /**
     * @notice Accepts payment for a message, reverting if not enough payment was provided.
     * @dev The `_handleGas` should cover the gas required by the recipient's `handle` function.
